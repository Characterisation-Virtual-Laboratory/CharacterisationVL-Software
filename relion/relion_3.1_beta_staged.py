...
##
# hpccm --recipe relion_3.1_beta_staged.py
#        --format singularity
...
# Adding in Relion 3.1_beta, build from source code.
...
Stage1 += environment(
    variables={
        ...
        "RELIONBINPATH": "/opt/relion-3.1_beta/bin",
        "RELIONLIBPATH": "/opt/relion-3.1_beta/lib:/usr/local/cuda/lib64:/usr/lib/x86_64-linux-gnu/openmpi/lib"
    },
    _export=False,
)
Stage1 += environment(
...
Stage1 += copy(_from=Stage0.name,
               src='/opt/relion-3.1_beta/bin/',
               dest='/opt/relion-3.1_beta/bin/')
Stage1 += copy(_from=Stage0.name,
               src='/opt/relion-3.1_beta/lib/',
               dest='/opt/relion-3.1_beta/lib/')
